...
describe("adding to the setlist fails", () => {
  it("keeps the server's own sentence when it refused the item itself", () => {
    const refusal = describeFailure(400, "Apologies cannot go in a service order");
    expect(notAdded(refusal)).toEqual({ refused: "Apologies cannot go in a service order" });
  });
...
describe("starting a setlist fails part way", () => {
  ...
  it("before anything exists: says it was not started, and keeps the name rule's own words", () => {
    expect(notStarted({ ...base, step: "create", failure: OFFLINE })).toEqual({ refused: `Service order not started. ${OFFLINE.message}` });
  ...
  it("created but not made active: names what exists and sends the operator to the Service orders page", () => {
    ...
    expect(text).toContain("Service orders page");
  });
